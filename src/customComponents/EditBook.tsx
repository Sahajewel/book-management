...
export default function EditBook() {
  ...
  if (isLoading) return  <div className="flex items-center justify-center py-10">
        <Loader2
          className={cn("h-10 w-10 animate-spin text-blue-600")}
        />
      </div>
  ...
  return (
    <div className="max-w-xl mx-auto p-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
       Title: <input className="w-full border p-2" name="title" value={formData.title} onChange={handleChange} />
        Author:<input className="w-full border p-2" name="author" value={formData.author} onChange={handleChange} />
        Genre:<input className="w-full border p-2" name="genre" value={formData.genre} onChange={handleChange} />
